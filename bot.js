...
async function ALFHA () {
    ...
    conn.on('connecting', async () => {
        console.log(`${chalk.green.bold('SL')}${chalk.blue.bold('ALFHA')}
${chalk.white.bold('Version:')} ${chalk.red.bold(config.VERSION)}

${chalk.blue.italic('⭕ Alfha WhatsApp වෙත සම්බන්ධ වෙමින් </> කරුණාකර රැඳී සිටින්න ❕\n⭕ Alfha Connecting to WhatsApp</> Please Wait ❕\n\n')}`);
    });
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('🤴 ප්ලගීන install කරන ලදි\n6️⃣ Plugins Installed ✅ \n\n🤴 ඇල්ෆා බොට් දැන් වැඩකරනු ලැබේ\n💌 Alfha Bot Now Working 🪀‍\n📌 කරුණාකර ඔබේ whatsapp ගිනුම පරික්ෂා කරන්න\n📌 Please check your whatsapp account\n\n')
        );

        if (config.LANG == 'SI') {
            await conn.sendMessage(conn.user.jid, fs.readFileSync("./rimg/r2.jpg"), MessageType.image, { caption: `☮ ALFHA Bot Now Working 💌\n\n👋 Hay ${conn.user.name} welcome to ALFHA bot 💗🙌\n\nJoin Our Bot Supported Group - https://chat.whatsapp.com/EVIVlBznGjXCiU5S8gzv8H \n\n🔮 _කරුණාකර මෙහි ප්ලගීන උත්සාහ නොකරන්න. මෙය ඔබගේ ලොග් අංකයයි. ⚙ ඔබට ඕනෑම චැට් එකක විධාන උත්සාහ කළ හැකිය :)_\n\n*☮ ALFHA Bot  භාවිතා කිරීම ගැන ස්තුතියි 💌*`});
            
        } else {
            await conn.sendMessage(conn.user.jid, fs.readFileSync("./rimg/r2.jpg"), MessageType.image, { caption: `☮ ALFHA Bot Now Working 💌\n\n👋 Hay ${conn.user.name} welcome to ALFHA bot 💗🙌\n\nJoin Our Bot Supported Group - https://chat.whatsapp.com/EVIVlBznGjXCiU5S8gzv8H \n\n🔮 _Please do not try plugins here. This is your login number. ⚙You can try the command in any chat :)_\n\n*💗 Thank You For Using Alfha Bot 💌`});
        }
    });
...
*/
                        try {
                            ...
                            if (config.LANG == 'SI') {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./rimg/Rs.jpg"), MessageType.image, { caption: '*⚕️  ඇල්ෆා බොට්හි දෝෂයක් සිදුවී ඇත  ⚕️*\n\n*ඇල්ෆා බොට්හි දෝෂයක් සිදුවී ඇත කරුණාකර එය අපගේ කණ්ඩායමට යොමු කරන්න* : *_https://chat.whatsapp.com/EVIVlBznGjXCiU5S8gzv8H*\n\n*දෝෂය:* ```' + error + '```\n\n' });
                                
                            } else {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./rimg/Rs.jpg"), MessageType.image, { caption: '*⚕️ ERROR ANALYSIS ALFHA BOT ⚕️*\n\n*An error has occurred in the ALFHA bot. Please refer it to our team* : *_https://chat.whatsapp.com/EVIVlBznGjXCiU5S8gzv8H*\n\n*error:* ```' + error + '```\n\n' });
                            }
                        }
                    }
                }
